...
uint256 hashGenesisBlock("0x92dfe9c2774741df3616d6bee0c40f5e940e7f3965c66f8d4fc73f7172eb168a");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Licoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Licoin Signed Message:\n";
...
{
    // Licoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    if(nHeight == 1){
        nSubsidy = nSubsidy * 200000000 * 50  / 100 / 500;
     }
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Licoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Licoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Licoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x92dfe9c2774741df3616d6bee0c40f5e940e7f3965c66f8d4fc73f7172eb168a");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1422700924;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085544815;
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("LicoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("LicoinMiner : ProcessBlock, block not accepted");
    }
...
void static LitecoinMiner(CWallet *pwallet)
{
    ...
    try { loop {
        ...
        printf("Running Licoin with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
